...
void helpShowAvailUpgrade(BinTree bt){
    if(bt!=NULL){
        if(bt->isApplied==false){
            printf("-> %d. ",bt->info);printkata(bt->detail);printf ("<Biaya pembangunan = %d, Durasi pembangunan= %d",bt->harga,bt->durasi);
...
void ShowAvailableUpgrade(ArrayWahana A,int IDWahana){
    ...
    if(D.id!=ValUndef){
        ...
        if(pohonUpgrade!=NULL){
            printf("upgrade available for "); printkata(D.wahana.nama); printf(" (ID wahana = %d)\n",D.id);
